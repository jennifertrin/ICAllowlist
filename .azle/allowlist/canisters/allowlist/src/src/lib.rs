...
#[ic_cdk_macros::query(name = "queryAddress")]
#[candid::candid_method(query, rename = "queryAddress")]
async fn _cdk_user_defined_queryAddress(_cdk_user_defined_address: String) -> (String) {
    BOA_CONTEXT_REF_CELL
        .with(|box_context_ref_cell| {
            let mut boa_context = box_context_ref_cell.borrow_mut();
            let uuid = uuid::Uuid::new_v4().to_string();
            ...
            METHOD_NAME_REF_CELL
                .with(|method_name_ref_cell| {
                    let mut method_name_mut = method_name_ref_cell.borrow_mut();
                    *method_name_mut = "queryAddress".to_string();
                });
            MANUAL_REF_CELL
                .with(|manual_ref_cell| {
                    let mut manual_mut = manual_ref_cell.borrow_mut();
            ...
            let function_js_value = exports_js_object
                .get("queryAddress", &mut boa_context)
                .unwrap();
